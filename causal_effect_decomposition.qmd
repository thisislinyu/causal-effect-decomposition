...
disparity_sim_f1 <-function(
  ...
  outcome_coef_vec,
  outcome_type ="cont",
  baseline_missing, ## true or false
  a = 0,
  b = 1,
  p = 0.5,
  seed=1017
 ...
 # }
  ...
  if(baseline_missing){
    ...
    miss_design_matrix <- model.matrix(~X2+Z, data = dat_X_Z)
    miss_coef_matrix <- c(-0.5,0.3,0.1,0)
    logit_val_X1 <- miss_design_matrix %*% miss_coef_matrix ## columns are the logit 
     exp_val_X1 <- apply(logit_val_X1,2,exp)
     denominator_X1 <-  1+ apply(exp_val_X1,1,sum)
     probX1_val <- (exp_val_X1/ denominator_X1) %>% data.frame()
     pop_dat$X1[probX1_val< 0.3] = NA
...
## MI imputation
library(mice)
mice0  <- mice(sim_miss_dat, seed = 100, m = 5, maxit = 0, print = F)
meth <- mice0$method

meth["X1"] <- "pmm" ## this is the default

pred <- mice0$predictorMatrix
pred[,] <- 0  
pred["X1", c("X2", "Z")] <- 1 ## use X2 and Z to predict X1

imputed_dat  <- mice(sim_miss_dat, seed = 100, m = 5, maxit = 10,
                        #,
                        method = meth,
...
## long data format
imputed_dat_long <- complete(imputed_dat, "long")
head(imputed_dat_long)

## list format

imputed_dat_list <- lapply(1:5, function(i) complete(imputed_dat, i))


## check and compare the first imputed dataset with the complete data
imputed_dat_list[[1]][which(is.na(sim_miss_dat$X1)),] %>% head()

sim_dat[which(is.na(sim_miss_dat$X1)),] %>% head()
...
if(outcome_type=="binary"){
 ...
 ###  end of the outcome model
  ...
  ### 🌈 change this to your hospital assignment model
  ...
  ##🌈 you may need to change the code based on the outcome model you have.
